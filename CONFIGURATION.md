...
- `CRYPBOT_BINGX_PUBLIC_MARKET_DATA_ENABLED=true` enables credential-free, read-only public prices.
- `CRYPBOT_BINGX_PUBLIC_MARKET_SYMBOLS=BTC-USDT,ETH-USDT,SOL-USDT` controls the explicit symbol allowlist.
...
The dashboard obtains BingX prices from `GET /api/v1/bingx/market/prices` even
while execution remains MOCK. A `503` means public data is disabled, the backend
cannot reach BingX, BingX rejected the response envelope, or a configured symbol
was omitted. It does not indicate that LIVE order execution should be enabled.
