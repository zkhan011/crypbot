...
class Settings(BaseSettings):
    ...
    bingx_public_market_data_enabled: bool = True
    bingx_public_market_symbols: str = "BTC-USDT,ETH-USDT,SOL-USDT"

    def public_market_symbols(self) -> tuple[str, ...]:
        symbols = tuple(dict.fromkeys(value.strip().upper() for value in self.bingx_public_market_symbols.split(",") if value.strip()))
        if not symbols or any(len(symbol) > 30 for symbol in symbols):
            raise RuntimeError("CRYPBOT_BINGX_PUBLIC_MARKET_SYMBOLS must contain valid symbols")
        return symbols
