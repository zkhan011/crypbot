...
function Dashboard() {
  ...
  const bingxMarket = useQuery({
    queryKey: ['bingx-public-market'],
    queryFn: () => api<BingXMarketResponse>('/api/v1/bingx/market/prices'),
    refetchInterval: 5000,
    retry: 1,
  });
    ...
    <main>
      ...
      <section>
        ...
        <section className="panel marketPanel">
          <h3>BingX public market prices</h3>
          <p>Read-only public REST data. Displaying prices does not activate LIVE trading or submit orders.</p>
          {bingxMarket.isError ? (
            <p className="error">BingX market data is unavailable. Check backend connectivity and CRYPBOT_BINGX_PUBLIC_MARKET_DATA_ENABLED.</p>
          ) : (
            <table>
              <thead><tr><th>Symbol</th><th>BingX price</th><th>Source</th></tr></thead>
              <tbody>
                {(bingxMarket.data?.prices ?? []).map((market) => (
                  <tr key={market.symbol}><td>{market.symbol}</td><td>{market.price}</td><td>{bingxMarket.data?.source}</td></tr>
                ))}
              </tbody>
            </table>
          )}
          <small>Last exchange tick: {bingxMarket.data?.timestamp ? new Date(bingxMarket.data.timestamp).toLocaleTimeString() : 'pending'}</small>
        </section>
